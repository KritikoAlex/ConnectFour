...
public class Networking {
    ...
    public void instruct() throws IOException, InterruptedException {
        ...
        if (new Scanner(System.in).nextLine().charAt(0) == 'y') {
            ...
            this.send(new Scanner(System.in).nextLine(), 135, new Packet(PacketType.CONNECT_TO_OPPONENT, 0));
            ...
            Listener.gotPacket = false;
            this.send(Listener.lastOpponentIp, 135, new Packet(PacketType.ACCEPT_INVITATION, 0));
            System.out.println("connected");
            ConnectFourMain.id = 1;
            Thread.sleep(250);
            this.send(Listener.lastOpponentIp, 135, new Packet(PacketType.TAKE_CONTROL, 0));
